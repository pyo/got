 ...
 */
function custom_meta_box_field( $field, $meta = null, $repeatable = null, $tag_select = null ) {
	...
	switch( $type ) {
		...
		case 'post_chosen':
			...
			$posts = get_posts( array( 'post_type' => $post_type, 'posts_per_page' => 99, 'orderby' => 'post_date', 'exclude'=>array(181890, 181891, 181893, 181896, 181897, 181898, 181899), 'order' => 'DESC', 'post_status'=>'publish' ) );
		...
		case 'tax_select':
			...
			$selected = $post_terms ? $post_terms[0]->slug : null;
		...
		case 'tag_select':
			echo '<select name="' . esc_attr( $name ) . '" id="' . esc_attr( $id ) . '">
					<option value="">Select One</option>'; // Select One
			$terms = ($tag_select !== null && is_array($tag_select)) ? $tag_select : get_terms( 'post_tag', array('orderby'=>'title') );
			$post_terms = wp_get_object_terms( get_the_ID(), 'post_tag' );
			$taxonomy = get_taxonomy( 'post_tag' );
			//$selected = $meta;
			echo 'Selected: '. $selected;
			foreach ( $terms as $term ) {
				$term_value = /*$taxonomy->hierarchical ? $term->term_id : */$term->term_id;
...
class Custom_Add_Meta_Box {
    ...
    public function __construct( $id, $title, $fields, $page, $js, $show_on ) {
		$this->id = $id;
		$this->title = $title;
		$this->fields = $fields;
		$this->page = $page;
		$this->show_on = $show_on ? $show_on : null;
		$this->terms = null;
		
		if( ! is_array( $this->page ) )
			$this->page = array( $this->page );

		if(in_array('tag_select', array_flatten(array_values($this->fields)))) $this->terms = get_terms( 'post_tag', array('orderby'=>'title') );
	...
	function add_box() {
		...
		foreach ( $this->page as $page ) {
			if( !is_null($this->show_on) ) {
				foreach ( $this->show_on as $key ) {
					if( $key === $post_id ) 
			...
			if ( $field['type'] == 'section' ) {
				...
				echo '<tr>
						...
						if($field['type'] == 'tag_select') {
							echo custom_meta_box_field( $field, $meta, null, $this->terms );
						} else {
		...
		foreach ( $this->fields as $field ) {
			...
			if( in_array( $field['type'], array( 'tax_select', 'tax_checkboxes' ) ) ) {
				...
				if ( isset( $_POST[$field['id']] ) ) {
					$term = $_POST[$field['id']];
					wp_set_object_terms( $post_id, $term, $field['id'], false );
				}
			}
			else {
				// save the rest
