			...
			<?php if(!empty($feature) && in_array('super_feature', $feature[0])): ?>
				...
				<li>
					<article data-post-id="<?php echo $post->ID; ?>" data-permalink="<?php echo $link; ?>" class="post-box super-feature">
						<div>
							<a class="thumb-wrapper" href="<?php echo esc_url( $link ); ?>"><?php if($format == 'video-format') echo '<img class="thumbnail-play-button" src="' . get_template_directory_uri() . '/img/icons/thumbnail_play_button.png">'; ?><img src="<?php echo tim_thumb_image(remove_http($featured_image), 483, 270, 100); ?>" class="lazy" alt="<?php the_title(); ?>" height="270" width="483"></a>
							<div class="details">
								<?php echo get_the_date(); //  get_time_since_posted(get_the_time('U', $post->ID)); ?><?php if(current_user_can('edit_post')): ?><?php edit_post_link( 'Edit', ' (', ') '); ?> <?php if (function_exists('wpp_get_views')) echo '| Views: ' . number_format( wpp_get_views( get_the_ID() ) ); ?><?php endif; ?>
					...
					<article data-post-id="<?php echo $post->ID; ?>" data-permalink="<?php echo $link; ?>" class="post-box">
						<div class="top">
							<div class="imgwrap">
								...
								<a class="thumb-wrapper" href="<?php echo esc_url( $link ); ?>"><?php if($format == 'video-format') echo '<img class="thumbnail-play-button" src="' . get_template_directory_uri() . '/img/icons/thumbnail_play_button.png">'; ?><img src="<?php echo tim_thumb_image(remove_http($featured_image), 202, 140, 100); ?>" class="lazy" alt="<?php the_title(); ?>" height="140" width="202"></a>
							</div>
							<div class="details">
								<?php echo get_the_date(); //  get_time_since_posted(get_the_time('U', $post->ID)); ?><?php if(current_user_can('edit_post')): ?><?php edit_post_link( 'Edit', ' (', ') '); ?> <?php if (function_exists('wpp_get_views')) echo '| Views: ' . number_format( wpp_get_views( get_the_ID() ) ); ?><?php endif; ?>
